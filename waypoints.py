"""Generate road-transport route waypoints by MAP-MATCHING the official GTFS to
OpenStreetMap (pfaedle), instead of mirroring the CSDI geometry.
...
sequence with a real map-matcher fixes every class at once.

Covers everything the GTFS carries as route_type 3 (KMB/CTB/LWB/NLB buses AND green
minibuses) in one pass; rail/tram/ferry keep their curated static files. Output is
`waypoints/{ROUTE_ID}-{O|I}.json` GeoJSON (ROUTE_ID == GTFS route_id; ROUTE_SEQ 1 ->
"O", else "I"; coords truncated to 5 dp) — unchanged for consumers.
...
# abort the run if pfaedle leaves more than this fraction unmatched
UNMATCHED_ABORT_RATE = 0.02
...
def seq_of(route_id: str, trip_id: str) -> str:
    ...
    # hyphen inside route_id can't be mistaken for the separator; seq 1 =
    # outbound, 2 = inbound.
...
def build_min_gtfs(src_dir: str, out_dir: str) -> dict:
    ...
    for t in rd("trips.txt"):
        rid = t["route_id"]
        if routes.get(rid, {}).get(
                "route_type") != "3":   # buses + green minibuses
            continue
        seq = seq_of(rid, t["trip_id"])
        if seq not in (
                "1", "2"):  # unexpected seq -> skip (never collide into O/I)
            skipped += 1
            continue
        kept.setdefault((rid, seq), t["trip_id"])
    if skipped:
        logging.warning(
            f"skipped {skipped} bus trips with unexpected trip_id seq (not 1/2)")
    ...
    wr("stops.txt",
       ["stop_id",
        ...
        "stop_lon"],
        [[sid,
          stops[sid].get("stop_name",
                         ""),
          stops[sid]["stop_lat"],
          stops[sid]["stop_lon"]] for sid in used_stops if sid in stops])
    wr("calendar.txt",
       ["service_id",
        "monday",
        "tuesday",
        ...
        [["D",
          ...
          "20261231"]])
    logging.info(
        f"minimal GTFS: {len(trip_meta)} route-directions, {len(used_stops)} stops")
    return trip_meta
...
def run_pfaedle(work: str):
    ...
    if cmd:
        args = cmd.split() + ["-x",
                              f"{work}/hong-kong.osm.pbf",
                              "-o",
                              f"{work}/out",
                              f"{work}/gtfs"]
    else:
        # write output as the host user so a non-root CI runner can clean up
        # the temp dir
        run_as = ["--user",
                  f"{os.getuid()}:{os.getgid()}"] if hasattr(os,
                                                             "getuid") else []
        args = [
            "docker",
            "run",
...
def write_waypoints(work: str, trip_meta: dict):
    ...
    with open(f"{work}/out/shapes.txt", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            shapes.setdefault(
                row["shape_id"], []).append(
                (int(
                    row["shape_pt_sequence"]), float(
                    row["shape_pt_lon"]), float(
                    row["shape_pt_lat"])))
    trip_shape = {}
    ...
    for tid, (rid, seq) in trip_meta.items():
        ...
        # must never happen (seq is 1/2 only) — fail loud
        if name in seen:
            raise RuntimeError(
                f"duplicate output filename {name}: direction/seq collision, aborting")
        seen.add(name)
...
def main():
    ...
    # non-root CI runner can't delete on teardown. The output files we need
    # are already written.
    ...
    with open("waypoints/0versions.json", "w", encoding="utf-8") as f:
        json.dump({"bus": version, "source": "pfaedle-osm",
                  "osm": OSM_URL}, f, indent=4)
